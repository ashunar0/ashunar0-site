...
```
  React が関数の外側に持っている記憶
  +----------------+
  | Counter state  |  <--  useState(0) が問い合わせる
  |   [0] = 3      |  -->  3 が返る
  +----------------+

  Counter 関数そのものは毎回まっさら
```
...
そしてこの「同じかどうか」の判定が、React で一番ハマるところだと思います。プログラミングスクールでメンターとしてインターンをしているんですが、かつて受講生に質問されたときに、うまく説明できずに濁したという苦い思い出があります。
...
+-------------------+        +--------------+
| newTodos: 0x9999  | -----> | [10, 20, 30] |  <-- 新しい実体
+-------------------+        +--------------+
```
